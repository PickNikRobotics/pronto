...
ScanMatcherHandler::ScanMatcherHandler(const rclcpp::Node::SharedPtr& node) : node_(node)
{
    Mode mode;
    ...
    if(!node_->get_parameter(prefix + "mode", mode_str)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"mode\". Using MODE_POSITION by default.");
        mode_str = "position";
    }
  ...
  if (mode_str.compare("position") == 0) {
    ...
    RCLCPP_INFO_STREAM(node_->get_logger(), "Scan matcher will provide position and yaw measurements.");
  }
  ...
  if (mode == Mode::POSITION || mode == Mode::POSITION_YAW || mode == Mode::POSITION_ORIENT)
  {
    ...
    if(!node_->get_parameter(prefix + "r_pxy", r_scan_match_pxy)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_pxy\". Using zero as default.");
    }
    double r_scan_match_pz = 0;
    if(!node_->get_parameter(prefix + "r_pz", r_scan_match_pz)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_pz\". Using zero as default.");
    }
    ...
    if(!node_->get_parameter(prefix + "r_yaw", r_scan_match_yaw)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_yaw\". Using zero as default.");
    }
    ...
    if(!node_->get_parameter(prefix + "r_vxy", r_scan_match_vxy)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_vxy\". Using zero as default.");
    }
    if(!node_->get_parameter(prefix + "r_vz", r_scan_match_vz)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_vz\". Using zero as default.");
    }
  ...
  if(mode == Mode::POSITION_ORIENT)
  {
    ...
    if(!node_->get_parameter(prefix + "r_rxy", r_scan_match_rxy)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_yaw\". Using zero as default.");
    }
    if(!node_->get_parameter(prefix + "r_ryaw", r_scan_match_ryaw)){
        RCLCPP_WARN(node_->get_logger(), "Couldn't get param \"r_yaw\". Using zero as default.");
    }
